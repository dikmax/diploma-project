...
class Thumbnail {
    ...
    public function __construct($stream) {
        ...
        if($this->error == false) {
            $this->oldImage = imagecreatefromstring($stream);
            $imageWidth = imagesx($this->oldImage);
            $imageHeight = imagesy($this->oldImage);

            $size = GetImageSize($this->fileName);
            $this->currentDimensions = array('width'=>$imageWidth,'height'=>$imageHeight);
            $this->newImage = $this->oldImage;
            $this->gatherImageMeta();
        }

        if($this->error == true) {
            throw new App_Exception('Image read error');
        }
    }
